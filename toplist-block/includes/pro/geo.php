...
/**
 * Append geo field to supported fields.
 *
...
/**
 * Filter render items by geo targeting.
 *
...
function toplist_geo_filter_render_items( array $items, array $attributes, int $context_id ): array {
	...
	$target = '';
	if ( 'custom' === $mode && isset( $attributes['geoCode'] ) && is_string( $attributes['geoCode'] ) ) {
		$target = strtoupper( sanitize_text_field( $attributes['geoCode'] ) );
	} elseif ( 'list' === $mode && 0 < $context_id ) {
