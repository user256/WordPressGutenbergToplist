...
/**
 * Register click tracking settings.
 *
 * @return void
 */
...
/**
 * Check if click obfuscation is enabled.
 *
 * @return bool
 */
 ...
 * @return string
 */
function toplist_outbound_link( string $url, string $content, string $css_class, int $list_id, int $row, string $kind ): string {
	if ( '' === $url ) {
		return '';
	}
	$html = '<a class="' . esc_attr( $css_class ) . '" rel="nofollow" target="_blank" href="' . esc_url( $url ) . '">' . $content . '</a>';
...
/**
 * Filter outbound link HTML for tracking.
 *
...
function toplist_filter_outbound_link_html( string $html, string $url, int $list_id, array $item ): string {
	...
	if ( toplist_click_obfuscation_enabled() ) {
		...
		return '<a href="#" class="toplist-outbound-obfuscated" role="link" data-toplist-outbound="' . esc_attr( base64_encode( $tracked ) ) . '" onclick="window.toplistFollowOutbound && window.toplistFollowOutbound(this);return false;">' . ( '' !== $label ? $label : esc_html__( 'Visit', 'toplist' ) ) . '</a>'; // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
	}
...
/**
 * Handle outbound click redirect.
 *
 * @return void
 */
function toplist_handle_outbound_click(): void {
	$list_param = isset( $_GET['list'] ) ? sanitize_text_field( wp_unslash( $_GET['list'] ) ) : 0;
	$row_param  = isset( $_GET['row'] ) ? sanitize_text_field( wp_unslash( $_GET['row'] ) ) : 0;
	...
	$to = isset( $_GET['to'] ) ? esc_url_raw( rawurldecode( sanitize_text_field( wp_unslash( $_GET['to'] ) ) ) ) : '';
	if ( '' === $to ) {
...
/**
 * Register REST route for click stats.
 *
 * @return void
 */
...
/**
 * Render click disclosure text after a list.
 *
 * @param int                  $list_id    List ID.
 * @param array<string, mixed> $attributes Attributes.
 * @return void
 */
function toplist_render_click_disclosure( int $list_id, array $attributes ): void { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
