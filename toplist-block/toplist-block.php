 ...
 * @return string
 */
function toplist_maybe_outbound_link(string $url, string $content, string $class, int $list_id, int $row, string $kind): string {
	if ($url === '') {
		return '';
	}
	if (function_exists('toplist_outbound_link')) {
		return toplist_outbound_link($url, $content, $class, $list_id, $row, $kind);
	}
	return '<a class="' . esc_attr($class) . '" rel="nofollow" target="_blank" href="' . esc_url($url) . '">' . $content . '</a>';
}
...
function toplist_render($attributes)
{
	...
	if ($card_layout_css !== '') {
		echo '<style>' . wp_strip_all_tags($card_layout_css) . '</style>';
	}
							...
							<?php if (($show_logo && $has_logo_content) || $has_identity_meta): ?>
								<div class="operator-column-logo-v2 logo-wrapper">
									<?php if ($show_logo && $has_logo_content): ?>
										<?php if ($href): ?>
											<?php
											echo toplist_maybe_outbound_link(
												$href,
												$logo
													? '<img class="op-logo" src="' . esc_url($logo) . '" loading="lazy" alt="' . esc_attr($product ?: $operator) . '" width="150" height="100" />'
													: '<span class="op-logo-fallback">' . esc_html($product ?: $operator) . '</span>',
						...
						<?php if ($has_play_column): ?>
							<div class="operator-playnow-column-v2">
								...
								<?php if ($has_cta): ?>
									...
									echo toplist_maybe_outbound_link(
										$href,
										'<span class="button-blue-v2">' . esc_html($ctaText) . '</span>',
										'operator-item__cta_link exit-page-link',
										$list_context_id,
										$pos,
										'cta'
