 ...
 * @return void
 */
function toplist_add_settings_page(): void {
	add_options_page(
 ...
 * @return void
 */
function toplist_register_settings(): void {
	register_setting(
		'toplist_settings',
		'toplist_global_css',
		array(
 ...
 * @return string
 */
function toplist_sanitize_css( string $css ): string {
 ...
 * @return void
 */
function toplist_settings_page(): void {
	...
	$bulk_import_requested = isset( $_POST['toplist_bulk_import_submit'] )
		|| 'toplist_import_all_csv' === $posted_action;
		...
		if ( ! isset( $_POST['toplist_import_all_csv_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( (string) wp_unslash( $_POST['toplist_import_all_csv_nonce'] ) ), 'toplist_import_all_csv' ) ) {
			...
			if ( UPLOAD_ERR_OK !== $upload_error ) {
				...
				if ( UPLOAD_ERR_NO_FILE === $upload_error ) {
					$status = 'empty';
				} elseif ( UPLOAD_ERR_INI_SIZE === $upload_error || UPLOAD_ERR_FORM_SIZE === $upload_error ) {
				...
				$tmp_file = isset( $_FILES['toplist_bulk_csv_file']['tmp_name'] ) ? sanitize_text_field( wp_unslash( $_FILES['toplist_bulk_csv_file']['tmp_name'] ) ) : '';
	...
	if ( isset( $_POST['toplist_save_settings'] ) ) {
		...
		update_option( 'toplist_global_css', toplist_sanitize_css( sanitize_textarea_field( wp_unslash( $_POST['toplist_global_css'] ?? '' ) ) ) );
	...
	$global_css                      = is_string( get_option( 'toplist_global_css', '' ) ) ? get_option( 'toplist_global_css', '' ) : '';
	$global_enable_default_header    = get_option( 'toplist_global_enable_default_header', '0' );
	$global_default_header_row       = is_string( get_option( 'toplist_global_default_header_row', '' ) ) ? get_option( 'toplist_global_default_header_row', '' ) : '';
	$global_heading_text             = is_string( get_option( 'toplist_global_heading_text', '' ) ) ? get_option( 'toplist_global_heading_text', '' ) : '';
	$global_default_cta_text         = is_string( get_option( 'toplist_global_default_cta_text', 'Visit' ) ) ? get_option( 'toplist_global_default_cta_text', 'Visit' ) : 'Visit';
	$global_default_read_review_text = is_string( get_option( 'toplist_global_default_read_review_text', 'Read Review' ) ) ? get_option( 'toplist_global_default_read_review_text', 'Read Review' ) : 'Read Review';
	// @toplist-premium-start
	...
	<div class="wrap">
		...
		if ( is_array( $local_bulk_result ) ) {
			...
			$bulk_updated       = (int) $local_bulk_result['updated'];
			$bulk_created       = (int) $local_bulk_result['created'];
			$bulk_rows          = (int) $local_bulk_result['rows'];
			$bulk_groups        = (int) $local_bulk_result['groups'];
		...
		if ( '' !== $bulk_import_status ) :
			...
			} elseif ( 'no_changes' === $bulk_import_status ) {
			...
			} elseif ( 'empty' === $bulk_import_status ) {
				$notice_class   = 'notice notice-warning';
					...
					<tr>
						...
						<td>
							<textarea name="toplist_global_css" id="toplist_global_css" rows="20" class="large-text code"
								style="font-family: monospace; font-size: 13px;"><?php echo esc_textarea( (string) $global_css ); ?></textarea>
							<p class="description">
						...
						<tr>
							...
							<td>
								<input type="text" class="regular-text" id="toplist_global_heading_text"
									...
									value="<?php echo esc_attr( (string) $global_heading_text ); ?>"
						...
						<tr>
							...
							<td>
								...
								<input type="text" class="large-text code" id="toplist_global_default_header_row"
									...
									value="<?php echo esc_attr( (string) $global_default_header_row ); ?>"
						...
						<tr>
							...
							<td>
								<input type="text" class="regular-text" id="toplist_global_default_cta_text"
									...
									value="<?php echo esc_attr( (string) $global_default_cta_text ); ?>">
						...
						<tr>
							...
							<td>
								<input type="text" class="regular-text" id="toplist_global_default_read_review_text"
									...
									value="<?php echo esc_attr( (string) $global_default_read_review_text ); ?>">
								...
								<div class="toplist-toggle-groups">
									<?php foreach ( $toggle_groups as $group_name => $group_fields ) : ?>
										<fieldset class="toplist-toggle-card">
											<h3><?php echo esc_html( (string) $group_name ); ?></h3>
											<?php foreach ( $group_fields as $option_name => $label ) : ?>
												<label>
													<input type="checkbox" class="toplist-toggle-checkbox" name="<?php echo esc_attr( (string) $option_name ); ?>" value="1"
	...
	add_action(
		...
		function () {
			...
			wp_register_style( 'toplist-global-css', false, array(), TOPLIST_BLOCK_VERSION );
			wp_enqueue_style( 'toplist-global-css' );
			wp_add_inline_style( 'toplist-global-css', is_string( $global_css ) ? $global_css : '' );
		}
	);
